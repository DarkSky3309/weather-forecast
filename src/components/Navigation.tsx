        ...
        <header className={"w-11/12 mx-auto flex justify-between text-white py-6 text-2xl"}>
            ...
            <div className={"flex gap-5"}>
                <div className={"relative"}>
                    <input value={searchCity} onChange={(e) => setSearchCity(e.target.value.toString())} type="text" placeholder={"Search..."} className={"text-xl text-black py-1 px-2 rounded-3xl"}/>
                    <button type={"submit"} onClick={()=>lookForCity()} className={"text-black bg-transparent border-none cursor-pointer inline-block absolute top-0 right-0 pr-2 hover:text-deep-cold duration-200"}><i className="ri-search-line"></i></button>
                </div>
